...
public interface DTableDao {
    ...
    @Insert("insert into dtablework(d_tab,d_num,d_type,d_content,d_basis,d_time,d_principal,d_way,d_remarks,dt_id,c_time,user_id) " +
            "values(#{tab},#{num},#{type},#{content},#{basis},#{time},#{principal},#{way},#{remarks},#{dt_id},#{c_time},#{user_id})")
    boolean InsertDTableID(@Param("tab") int tab, @Param("num") int num, @Param("type") int type,
                         @Param("content") String content, @Param("basis") String basis, @Param("time") String time,
                         @Param("principal") String principal, @Param("way") String way, @Param("remarks") String remarks,
                           @Param("dt_id") double dt_id,@Param("c_time") String c_time, @Param("user_id") int user_id);
    ...
    @Select("select newnum from curdata")
    public int GetNewNum();

    @Update("update curdata set newnum = #{newnum}")
    public boolean addNewNum(@Param("newnum") int newnum);
